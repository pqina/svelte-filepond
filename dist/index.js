(function (global, factory) {
    ...
    const attr$1 = (node, name, value = null) => {
      if (value === null) {
    ...
    const createElement = (tag, className, attributes = {}) => {
      ...
      if (className) {
        if (isSVGElement(tag)) {
          attr$1(element, 'class', className);
        } else {
          element.className = className;
        }
      }
      forin(attributes, (name, value) => {
        attr$1(element, name, value);
      });
      return element;
    };
    ...
    */
    ...
    const write = ({ root, props }) => {
      ...
      // get width of stroke
      const ringStrokeWidth = parseInt(attr$1(root.ref.path, 'stroke-width'), 10);
      ...
      // update progress bar
      attr$1(root.ref.path, 'd', coordinates);
    ...
    const write$1 = ({ root, props }) => {
      ...
      if (shouldDisable && !isDisabled) {
        ...
        attr$1(root.element, 'disabled', 'disabled');
    ...
    const create$2 = ({ root, props }) => {
      ...
      attr$1(fileName, 'aria-hidden', 'true');
    ...
    const attrToggle = (element, name, state, enabledValue = '') => {
      if (state) {
        attr$1(element, name, enabledValue);
      } else {
    ...
    const create$a = ({ root, props }) => {
      ...
      attr$1(root.element, 'name', root.query('GET_NAME'));

      // we have to link this element to the status element
      attr$1(root.element, 'aria-controls', `filepond--assistant-${props.id}`);

      // set label, we use labelled by as otherwise the screenreader does not read the "browse" text in the label (as it has tabindex: 0)
      attr$1(root.element, 'aria-labelledby', `filepond--drop-label-${props.id}`);
    ...
    const create$b = ({ root, props }) => {
      ...
      attr$1(label, 'for', `filepond--browser-${props.id}`);

      // use for labeling file input (aria-labelledby on file input)
      attr$1(label, 'id', `filepond--drop-label-${props.id}`);

      // hide the label for screenreaders, the input element will read the contents of the label when it's focussed. If we don't set aria-hidden the screenreader will also navigate the contents of the label separately from the input.
      attr$1(label, 'aria-hidden', 'true');
    ...
    const create$d = ({ root, props }) => {
      ...
      attr$1(root.element, 'role', 'status');
      attr$1(root.element, 'aria-live', 'polite');
      attr$1(root.element, 'aria-relevant', 'additions');
    };
    ...
    const getAttributesAsObject = (node, attributeMapping = {}) => {
      ...
      const output = attributes
        ...
        .reduce((obj, attribute) => {
          const value = attr$1(node, attribute.name);
    ...
    if (supported()) {
      ...
      exports.registerPlugin = (...plugins) => {
        // register plugins
    ...
    function add_css() {
    	...
    	style.id = "svelte-ilgcgs-style";
    	style.textContent = "@import 'filepond/dist/filepond.min.css';";
    ...
    function create_fragment(ctx) {
    	...
    	return {
    		c() {
    			...
    			attr(input, "class", /*klass*/ ctx[0]);
    			attr(input, "accept", /*acceptedFileTypes*/ ctx[6]);
    			input.multiple = /*allowMultiple*/ ctx[3];
    			input.required = /*required*/ ctx[4];
    			attr(input, "capture", /*captureMethod*/ ctx[5]);
    			attr(div, "class", "filepond--wrapper");
    		},
    		m(target, anchor) {
    		...
    		p(ctx, [dirty]) {
    			...
    			if (dirty & /*acceptedFileTypes*/ 64) {
    				attr(input, "accept", /*acceptedFileTypes*/ ctx[6]);
    			}

    			if (dirty & /*allowMultiple*/ 8) {
    				input.multiple = /*allowMultiple*/ ctx[3];
    			}
    ...
    function instance_1($$self, $$props, $$invalidate) {
    	...
    	let { captureMethod = undefined } = $$props;
    	let { acceptedFileTypes = undefined } = $$props;
    	...
    	afterUpdate(() => {
    		...
    		if (!instance) {
    			// create instance
    			instance = create$f(root, { ...$$props });

    			// Reference pond methods to FilePond component instance
    			$$invalidate(8, addFile = instance.addFile);

    			$$invalidate(9, addFiles = instance.addFiles);
    			$$invalidate(19, removeFile = instance.removeFile);
    			$$invalidate(20, removeFiles = instance.removeFiles);
    			$$invalidate(10, browse = instance.browse);
    			$$invalidate(11, fireSync = instance.fireSync);
    			$$invalidate(12, getFile = instance.getFile);
    			$$invalidate(13, getFiles = instance.getFiles);
    			$$invalidate(14, moveFile = instance.moveFile);
    			$$invalidate(15, prepareFile = instance.prepareFile);
    ...
    class Component extends SvelteComponent {
    	constructor(options) {
    		super();
    		if (!document.getElementById("svelte-ilgcgs-style")) add_css();
